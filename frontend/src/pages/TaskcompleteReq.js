...
import { IoSearchOutline, IoCheckmarkDoneOutline } from "react-icons/io5";
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      tasks = tasks.filter((task) => {
        const matchesDateOfRequest = task.date_of_request
          ? new Date(task.date_of_request).toLocaleString().toLowerCase().includes(searchLower)
          : false;
        ...
        const matchesProjectName = task.project_name
          ? task.project_name.toLowerCase().includes(searchLower)
          : false;
  
        return (
          matchesDateOfRequest ||
          matchesStatus ||
          matchesUserEmail ||
          matchesTaskName ||
          matchesProjectName
